...
def generate_models(x, y, degs):
    ...
    models = []
    for d in degs:
        models.append(np.array(np.polyfit(x,y,d)))
...
def r_squared(y, estimated):
    ...
    """
    sum_diffs_sq = 0.0
    sum_mean_diffs_sq = 0.0
    y_mean = np.average(y)

    for i in range(len(y)):
        sum_diffs_sq += (y[i] - estimated[i])**2
        sum_mean_diffs_sq += (y[i] - y_mean)**2

    r_sq = 1 - (sum_diffs_sq/sum_mean_diffs_sq)
...
def evaluate_models_on_training(x, y, models):
    ...
    for i in range(len(models)):
        pylab.plot(x, y, label="Actual", marker='o', c='blue', ls='none')
        estimated = pylab.polyval(models[i],x)
        pylab.plot(x, estimated, label="Model_" + str(i), c='red', ls = '-')
        r_sq = r_squared(y,estimated)
        pylab.title("Model_"+ str(i) + " evaluation. R_sq=" + str(r_sq))
